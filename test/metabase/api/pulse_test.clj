  ...
  (testing "POST /api/pulse"
    ...
    (testing "dashboard subscriptions"
      (mt/with-temp* [Collection [collection]
                      Card [card-1]
                      Card [card-2]
                      Dashboard [{permitted-dashboard-id :id} {:name "Birdcage KPIs" :collection_id (:id collection)}]
            ...
            (mt/with-model-cleanup [Pulse]
              (is (= (merge
                      ...
                      {:name          "A Pulse"
                       ...
                       :collection_id true
                       :dashboard_id permitted-dashboard-id})
                     (-> ((mt/user->client :rasta) :post 200 "pulse" payload)
                         pulse-response
                         (update :channels remove-extra-channels-fields))))
              (is (= "You don't have permissions to do that."
                     ((mt/user->client :rasta) :post 403 "pulse" (assoc payload :dashboard_id blocked-dashboard-id)))))))))))
